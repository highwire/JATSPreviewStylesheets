<xsl:stylesheet version="1.0"
  ...
  <xsl:param name="book-type" select="$atom-sequence[4]"/>
  <xsl:param name="book-id">
    <xsl:choose>
      <xsl:when test="$atom/atom:entry/atom:category[@scheme eq 'http://schema.highwire.org/ItemSet/Item#type'][starts-with(@term,'book/')]">
        <xsl:value-of select="substring-before($atom-sequence[5],'.atom')"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="$atom-sequence[5]"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:param>
  ...
  <xsl:param name="book-atom"
    select="
      if (matches($atom-id,'monograph'))
      then document(resolve-uri(concat('/ersworks/book/monograph/',$book-id,'.atom'),base-uri(.)))
      else document(resolve-uri(concat('/ersworks/book/handbook/',$book-id,'.atom'),base-uri(.)))"/>
  <xsl:param name="cover-source"
    select="
      if (matches($atom-id,'monograph'))
      then document(resolve-uri(concat('/ersworks/book/monograph/',$book-id,'/cover/supplementary-material1.source.xml'),base-uri(.)))
      else document(resolve-uri(concat('/ersworks/book/handbook/',$book-id,'/cover/supplementary-material1.source.xml'),base-uri(.)))"/>
  ...
  <xsl:template match="/">
    <html>
      ...
      <xsl:if test="$source/book-part/(book-part-meta|book-meta)/lpage">
        <meta name="citation_lastpage" content="{$source/book-part/(book-part-meta|book-meta)/lpage}"/>
      </xsl:if>
      <meta name="citation_xml_url" content=""/>
      <meta name="citation_inbook_title" content="{doc(concat('http://atom.highwire.org/ersworks/book/',$book-type,'/',$book-id,'.atom'))/atom:entry/atom:title}"/>
      ...
      <xsl:apply-templates select="//aff" mode="hw"/>
  ...
  <xsl:template match="aff" mode="hw">
    <xsl:variable name="affiliation" select="normalize-space(.)"/>
